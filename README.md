...
也可以用浏览器打开 [AI × Security 全栈工程实战手册](docs/ai-security-field-guide.html)：
包含完整七天操作、四张流程图、排障步骤、AI 安全实验和十二周进阶建议。
单文件支持离线阅读、复制命令、保存勾选与笔记、导出学习记录及打印；
页面进度保存在当前浏览器，正式实验结论仍记录到上述实验日志。
