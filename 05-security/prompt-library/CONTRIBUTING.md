...
2. **YAML frontmatter is required for new skills.** The exporter also supports the existing legacy Markdown metadata format.
...
5. **Use code blocks with language tags.** They make examples easier to interpret.
...
description: "Briefly state the surface and when to use this skill. Prefer one or two precise sentences over a long inventory of techniques."
---
```

The `description` helps select the skill. Keep exported descriptions under 1024 characters. Put detailed procedures in the body. The dependency-free exporter accepts string metadata, including quoted strings and folded descriptions; extend its parser and tests before introducing other YAML structures.
...
6. Update [`skills.json`](skills.json) with `python3 tools/build_manifest.py`.
7. Validate conversion and installation:
   ```bash
   python3 -m unittest discover -s tests
   bash -n install.sh
   python3 convert_skills.py --dry-run
   ```
...
- AI-generated skills without operator review (verify and edit generated drafts)
